...
(reg-event-fx ::load-from-ls
              ...
              (fn [cofx _]
                {:db (:db cofx)
                 :fx [[:dispatch [::set-canvas (:canvas-from-ls cofx)]]]}))
                      ...
                      (update-in [::canvas canvas-id :render-items render-id]
                                 (fn [render-item]
                                   (let [render-item*
                                     ...
                                     (calculate-target-basis translation render-item*))))))))

;; ********************************************************************************
;; Basis translation

(defn- calculate-translation
  "A translation from `source` to `target` is calculated"
  [db canvas-id]
...
(defn- on-target-basis-change
  "To be called when the target basis has changed"
  [db canvas-id]
  (let [translation (calculate-translation db canvas-id)]
    (update-in db
               [::canvas canvas-id]
               (fn [canva]
                 (cond-> canva
                   true (assoc-in [:basis :translation] translation)
                   (:render-items canva) (update :render-items
                                                 update-vals
                                                 (partial calculate-target-basis translation)))))))
...
(defn layout*
  [opts canvas-id {:keys [multi-select-mode?]} render-items]
  (r/create-class
   {:display-name "Simulation layout"
    ...
    (fn []
      (let [drag-render-element-data @(subscribe [::drag-render-element-data canvas-id])
            ...
            ;;render-items (or @(subscribe [::render-items canvas-id]) render-items)
           ]
        [:div
         (-> opts
             (update :style assoc
...
(defn layout
  ...
  (if (map? opts) (layout* opts canvas-id pars render-items) (layout* {} opts canvas-id pars)))
