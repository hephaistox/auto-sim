(ns auto-sim.rc.impl.resource.queue-test
  (:require
   ...
   [auto-sim.rc.impl.resource.queue            :as sut]
...
(deftest queue-event-test
  ...
  (testing "The first event is added in the empty queue"
    (is (= #:auto-sim.rc{:queue [#:auto-sim.rc{:seizing-event #:auto-sim.simulation-engine{:type :a
                                                                                           :date 1}
                                               :consumed-quantity 13}]
                         :resource-name ::test}
           (sut/queue-event #:auto-sim.rc{:resource-name ::test}
                            13
...
(deftest unqueue-event-test
  (testing "Non integer `available-capacity` is ok"
    (is (= [[] {::sim-rc/queue []}]
           ...
           (unqueue-event-cacheproof
            #:auto-sim.rc{:queue [{::sim-rc/seizing-event [#:auto-sim.simulation-engine{:type :a
                                                                                        :date 1}
                                                           #:auto-sim.simulation-engine{:type :b
                                                                                        :date 2}]}]}
            nil))))
  (testing "Unqueue empty queue is ok"
    (is (= [[] #:auto-sim.rc{:queue []}] (unqueue-event-cacheproof nil 1))))
  (testing
    "Unqueue a non positive integer of `available-capacity` in a non empty queue is removing one and only one event"
    (is (= [[] #:auto-sim.rc{:queue [{:a 2}]}]
           (unqueue-event-cacheproof #:auto-sim.rc{:queue [{:a 2}]} 0))))
  (testing "Unqueue takes the first event and leave next ones in the queue"
    (is (= [[#:auto-sim.rc{:seizing-event #:auto-sim.simulation-engine{:type :a
                                                                       :date 1}}]
            #:auto-sim.rc{:queue [#:auto-sim.rc{:seizing-event #:auto-sim.simulation-engine{:type :b
                                                                                            :date
                                                                                            2}}]}]
           (unqueue-event-cacheproof
            #:auto-sim.rc{:queue
                          [#:auto-sim.rc{:seizing-event #:auto-sim.simulation-engine{:type :a
                                                                                     :date 1}}
                           #:auto-sim.rc{:seizing-event #:auto-sim.simulation-engine{:type :b
                                                                                     :date 2}}]}
            1)))
    (is (= [[#:auto-sim.rc{:seizing-event #:auto-sim.simulation-engine{:type :a
                                                                       :date 1}
            ...
            #:auto-sim.rc{:queue [#:auto-sim.rc{:seizing-event #:auto-sim.simulation-engine{:type :b
                                                                                            ...
                                                                                            2}}]}]
           (unqueue-event-cacheproof
            #:auto-sim.rc{:queue [#:auto-sim.rc{:seizing-event #:auto-sim.simulation-engine{:type :a
                                                                                            :date 1}
                                                :consumed-quantity 1}
                                  #:auto-sim.rc{:seizing-event #:auto-sim.simulation-engine{:type :b
                                                                                            :date
  ...
  (testing "Thread queue and unqueue to check unqueueing is finding the first queued element first"
    ...
    (is (= [[#:auto-sim.rc{:seizing-event {:a :b}
            ...
            #:auto-sim.rc{:queue [#::sim-rc{:seizing-event {:a :c}
                                            :consumed-quantity 19}
                                  #:auto-sim.rc{:seizing-event {:d :b}
  ...
  (testing "unqueue less than the first capacity"
    (is (= [[]
            #:auto-sim.rc{:queue [#::sim-rc{:seizing-event {:a :b}
  ...
  (testing "unqueue exactly the expected capacity of two events"
    ...
    (is (= [[#:auto-sim.rc{:seizing-event {:a :b}
                           :consumed-quantity 1}]
            #:auto-sim.rc{:queue [#::sim-rc{:seizing-event {:d :b}
                                            :consumed-quantity 2}]}]
           (-> {}
               (sut/queue-event 1 {:a :b})
               (sut/queue-event 2 {:d :b})
    ...
    (is (= [[#:auto-sim.rc{:seizing-event {:a :b}
                           :consumed-quantity 1}
             #:auto-sim.rc{:seizing-event {:b :b}
                           :consumed-quantity 2}]
            #:auto-sim.rc{:queue [#::sim-rc{:seizing-event {:c :b}
                                            :consumed-quantity 4}
                                  #:auto-sim.rc{:seizing-event {:d :b}
                                                :consumed-quantity 2}]}]
           (-> {}
