(ns auto-sim.rc.impl.resource-test
  (:require
   [auto-sim.rc                          :as sim-rc]
   #?(:clj [clojure.test :refer [deftest is testing]]
      :cljs [cljs.test :refer [deftest is testing] :include-macros true])
   [auto-sim.rc.impl.resource            :as sut]
   [auto-sim.rc.impl.resource.queue-test :as sim-de-resource-queue-test]
...
(deftest nb-available-resources-test
  ...
  (testing "With no currently seized, all the capacity is available"
    (is (= 7 (sut/nb-available-resources #:auto-sim.rc{:capacity 7}))))
  (testing
...
(deftest seize-test
  ...
  (testing "Seizing one resource with capacity missing"
    (is (nil? (first (sut/seize #:auto-sim.rc{:capacity 0} 1 {:a :b}))))
    (is (nil? (first (sut/seize #:auto-sim.rc{:capacity 12} 20 {:a :b}))))
    (is (empty? (-> (sut/seize #:auto-sim.rc{:capacity 12} 20 {:a :b})
                    second
...
(deftest dispose-seize-test
  (testing "Disposing 1 which is not enough for next event"
    (let [resource (-> #:auto-sim.rc{:capacity 1}
                       (sut/seize 1 {:a :b1})
                       second
                       (sut/seize 2 {:a :b2})
                       second
